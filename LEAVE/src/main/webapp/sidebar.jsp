...
<aside id="appSidebar" class="fixed left-0 top-0 h-full bg-[#0f172a] text-slate-200 border-r border-slate-800 z-50 flex flex-col w-20 lg:w-64 transition-all duration-300">
    ...
    <nav class="flex-1 px-2 mt-4 space-y-1.5 no-scrollbar overflow-y-auto">
        <% if (isAdmin) { %>
            ...
            <a href="RegisterEmployee" class="nav-item flex items-center gap-3 px-3 py-2.5 mx-2 rounded-xl <%= (pageKey.contains("employee") || pageKey.contains("register")) ? "active-blue" : "text-slate-400 hover:bg-slate-800/50 hover:text-white" %>">
            ...
            <a href="leaveEmpHistory" class="nav-item flex items-center gap-3 px-3 py-2.5 mx-2 rounded-xl <%= pageKey.equals("leaveemphistory") ? "active-blue" : "text-slate-400 hover:bg-slate-800/50 hover:text-white" %>">
                <span class="shrink-0"><%= ClipboardListIcon("w-5 h-5") %></span>
                <span class="hidden lg:block whitespace-nowrap text-sm font-semibold">Leave History</span>
            </a>

            <a href="ManageHoliday" class="nav-item flex items-center gap-3 px-3 py-2.5 mx-2 rounded-xl <%= (pageKey.contains("holiday")) ? "active-blue" : "text-slate-400 hover:bg-slate-800/50 hover:text-white" %>">
        ...
        <% } else { %>
            ...
            <a href="ApplyLeave" class="nav-item flex items-center gap-3 px-3 py-2.5 mx-2 rounded-xl <%= pageKey.contains("apply") ? "active-blue" : "text-slate-400 hover:bg-slate-800/50 hover:text-white" %>">
                <span class="shrink-0"><%= CalendarIcon("w-5 h-5") %></span>
                <span class="hidden lg:block whitespace-nowrap text-sm font-semibold">Apply Leave</span>
            </a>

            <a href="LeaveHistory" class="nav-item flex items-center gap-3 px-3 py-2.5 mx-2 rounded-xl <%= (pageKey.equals("leavehistory")) ? "active-blue" : "text-slate-400 hover:bg-slate-800/50 hover:text-white" %>">
                <span class="shrink-0"><%= ClipboardListIcon("w-5 h-5") %></span>
                <span class="hidden lg:block whitespace-nowrap text-sm font-semibold">My History</span>
            </a>
        <% } %>
    </nav>
</aside>