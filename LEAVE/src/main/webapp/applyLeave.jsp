...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; font-family: 'Inter', Arial, sans-serif !important; }
    ...
    .duration-options { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; height: 54px; }
    .duration-tile {
    ...
    .duration-tile input { display: none !important; }
    .duration-tile span { font-size: 10px; font-weight: 900; color: #64748b; text-transform: uppercase; letter-spacing: 0.02em; }
    .duration-tile.selected { border-color: var(--primary); background: #eff6ff; }
    ...
    .overlay { position: fixed; inset: 0; background: rgba(15, 23, 42, 0.7); display: none; align-items: center; justify-content: center; z-index: 9999; backdrop-filter: blur(8px); padding: 20px; }
    ...
    .btn-modal-cancel { background: #f1f5f9; color: #64748b; font-weight: 800; }
...
<body class="flex">
  ...
  <main class="flex-1 ml-20 lg:ml-64 min-h-screen transition-all duration-300">
    ...
    <div class="pageWrap">
      ...
      <div class="card">
        ...
        <form action="ApplyLeave" method="post" enctype="multipart/form-data" id="applyForm" onsubmit="return handleApplyForm(event)">
          ...
          <div class="mb-8">
            ...
            <textarea name="reason" id="reason" required placeholder="EXPLAIN WHY YOU ARE TAKING THIS LEAVE..."></textarea>
          </div>
  ...
  <div class="overlay" id="confirmOverlay">
    <div class="modal">
      ...
      <h3 class="text-xl font-black uppercase tracking-tight text-slate-900">Confirm Submission</h3>
      <p class="text-slate-500 text-sm mt-3 mb-10 leading-relaxed font-medium">Are you sure you want to submit this <b class="text-slate-900 uppercase" id="confirmTypeSpan"></b> request? Please ensure all details are accurate.</p>
      
      <div class="flex gap-3">
        <button type="button" onclick="closeConfirmModal()" class="btn-submit btn-modal-cancel flex-1">Cancel</button>
        <button type="button" onclick="submitFinalForm()" class="btn-submit flex-1 shadow-lg shadow-blue-100">Confirm</button>
      </div>
    </div>
  </div>
  ...
  <div class="overlay" id="successOverlay">
    <div class="modal">
      <div class="bg-emerald-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 text-emerald-500">
        <%= CheckCircleIcon("w-10 h-10") %>
      </div>
      <h3 class="text-xl font-black uppercase tracking-tight text-slate-900">Success!</h3>
      <p class="text-slate-500 text-sm mt-3 mb-10 leading-relaxed font-medium uppercase">Your leave application has been submitted and is pending review by your manager.</p>
      <button class="btn-submit" onclick="location.href='LeaveHistory'">Go to History</button>
    </div>
  </div>
