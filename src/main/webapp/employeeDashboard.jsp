...
// Semakan sesi untuk mengelakkan NullPointerException
...
String fullname = (ses.getAttribute("fullname") != null) ? String.valueOf(ses.getAttribute("fullname")) : "User";

// Pengurusan ralat pangkalan data
...
// Mapping baki cuti mengikut kod jenis cuti
...
// Menentukan hari pertama dalam minggu (0=Ahad untuk grid Malaysia/Satu-Satu)
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; font-family: 'Inter', sans-serif !important; }
        ...
        .pageWrap { padding: 24px 20px; display: flex; flex-direction: column; width: 100%; }
        @media (min-width: 768px) { .pageWrap { padding: 32px 30px; } }
        ...
        .card { background: var(--card); border-radius: 16px; border: 1px solid var(--border); box-shadow: var(--shadow); padding: 16px 20px; position: relative; display: flex; flex-direction: column; transition: all 0.2s ease; border-left-width: 5px; min-height: 170px; }
        .card:hover { transform: translateY(-2px); box-shadow: 0 12px 20px -5px rgba(0, 0, 0, 0.08); }
        ...
        .label-badge { font-size: 9px; font-weight: 900; color: #1e293b; text-transform: uppercase; letter-spacing: .05em; background: rgba(255, 255, 255, 0.8); padding: 3px 10px; border-radius: 8px; border: 1px solid #e2e8f0; }
        .card .big { font-size: 26px; font-weight: 900; color: #000; margin-top: 6px; line-height: 1; }
        .card .big .slash { color: #cbd5e1; margin: 0 2px; }
        .card-footer { border-top: 1.5px solid #f1f5f9; padding-top: 12px; margin-top: auto; }
        .stats-row { display: flex; align-items: center; justify-content: space-between; }
        .stat-box span { color: var(--muted); font-size: 9px; text-transform: uppercase; font-weight: 800; display: block; margin-bottom: 2px; }
        .stat-box b { color: #1e293b; font-size: 14px; font-weight: 900; }
        .cal-card { background: #fff; border: 1px solid var(--border); border-radius: var(--radius); padding: 24px; box-shadow: var(--shadow); }
        .calHeader { display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; }
        .calTitle { font-weight: 950; font-size: 17px; color: #0f172a; letter-spacing: -0.02em; }
        .calTable { width: 100%; text-align: center; }
        .calTable th { font-size: 11px; color: #94a3b8; font-weight: 900; padding-bottom: 12px; text-transform: uppercase; }
        .dayBox { display: inline-flex; align-items: center; justify-content: center; width: 38px; height: 38px; border-radius: 12px; font-weight: 900; font-size: 13px; transition: 0.2s; cursor: pointer; margin-bottom: 2px; }
        .today { background: #000 !important; color: #fff !important; }
        .tipWrap { position: relative; display: inline-block; }
        .tip { position: absolute; bottom: 125%; left: 50%; transform: translateX(-50%); background: #0f172a; color: #fff; padding: 6px 10px; border-radius: 8px; font-size: 10px; white-space: nowrap; opacity: 0; pointer-events: none; transition: 0.2s; z-index: 100; font-weight: 700; }
        ...
        .hListItem { display: flex; gap: 12px; align-items: center; padding: 12px 0; border-bottom: 1px solid #f8fafc; }
        .dateBadge { width: 44px; height: 44px; border-radius: 12px; flex-shrink: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; background: #f8fafc; border: 1px solid var(--border); }
        .dateBadge span:first-child { font-size: 16px; font-weight: 950; line-height: 1; }
        .dateBadge span:last-child { font-size: 9px; font-weight: 800; text-transform: uppercase; }
        ...
        #clock { font-size: 28px; font-weight: 950; color: var(--text); font-variant-numeric: tabular-nums; line-height: 1; }
        #date { font-size: 10px; font-weight: 700; color: var(--muted); text-transform: uppercase; margin-top: 4px; }
    </style>
</head>
        ...
        <div class="pageWrap">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-6">
                <div>
                    ...
                    <span class="sub-label"> Welcome back <span class="text-[15px] font-black"><%=fullname%></span> <br> Access your leave summary and calendar</span>
                </div>
                <div class="clock-box bg-white/60 px-6 py-3 rounded-2xl border border-white shadow-sm text-right">
            ...
            <div class="flex flex-col lg:flex-row gap-10 items-start">
                ...
                <div class="w-full lg:flex-1 grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    String[] typesOrder = {"ANNUAL", "SICK", "EMERGENCY", "HOSPITALIZATION", "UNPAID", "MATERNITY", "PATERNITY"};
                    ...
                    <div class="card <%=cardTheme%>">
                        <div class="flex justify-between items-start mb-3">
                            <div class="w-9 h-9 bg-white/80 rounded-xl border border-slate-100 flex items-center justify-center text-slate-600 shadow-sm">
                                <%=CalendarIcon("w-5 h-5")%>
                            </div>
                            <span class="label-badge"><%=type.replace("_", " ")%></span>
                        </div>
                        <div class="mt-1">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Available Balance</span>
                            <div class="big flex items-baseline">
                                <span><%=df.format(b.getTotalAvailable())%><span class="slash">/</span><%=df.format(b.getEntitlement())%></span>
                                <span class="text-[11px] font-black text-slate-400 uppercase ml-2">Days</span>
                            </div>
                        </div>
                        <div class="card-footer">
                            <div class="stats-row">
                                <div class="stat-box"><span>Taken</span> <b><%=df.format(b.getUsed())%></b></div>
                                <div class="stat-box text-right"><span>Pending</span> <b class="text-orange-500"><%=df.format(b.getPending())%></b></div>
                            </div>
                        </div>
                    </div>
                    <% } %>
                ...
                <div class="w-full lg:w-[420px] flex flex-col gap-8">
                    <div class="cal-card">
                        <div class="calHeader">
                            ...
                            <div class="flex gap-2">
                                <a href="EmployeeDashboard?year=<%=prev.getYear()%>&month=<%=prev.getMonthValue()%>" class="w-9 h-9 flex items-center justify-center border border-slate-100 rounded-xl hover:bg-slate-50 shadow-sm bg-white">
                                    <%=ChevronLeftIcon("w-4 h-4 text-slate-500")%>
                                </a>
                                <a href="EmployeeDashboard?year=<%=next.getYear()%>&month=<%=next.getMonthValue()%>" class="w-9 h-9 flex items-center justify-center border border-slate-100 rounded-xl hover:bg-slate-50 shadow-sm bg-white">
                        ...
                        <table class="calTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="tipWrap">
                                            <span class="dayBox <%=isToday ? "today shadow-lg" : "hover:bg-slate-100 text-slate-600"%>">
                                                <%=dayCounter%>
                                            </span>
                                            <% if (isHoliday) { 
                                                String hType = hs.get(0).getType().toUpperCase();
                                                String dotClass = hType.contains("PUBLIC") ? "h-public-dot" : (hType.contains("STATE") ? "h-state-dot" : "h-company-dot");
                                            %>
                                            <div class="h-dot <%=dotClass%>"></div>
                                            <span class="tip"><%=hs.get(0).getName()%></span>
                                            <% } %>
                                        </div>
                                    </td>
                                    <% dayCounter++; } } %>
                    ...
                    <div class="cal-card flex flex-col">
                        <h3 class="font-black text-[12px] uppercase text-slate-400 tracking-widest mb-4 flex items-center gap-2 border-b pb-4 border-slate-50 shrink-0">
                            <%=CalendarIcon("w-4 h-4 text-blue-500")%> Upcoming Holidays
                        </h3>
                        <div class="space-y-1">
                            <%
                            int count = 0;
                            ...
                            %>
                            <div class="hListItem hover:bg-slate-50 px-2 rounded-xl border-none">
                                <div class="dateBadge <%=badgeCls%>">
                                    <span><%=h.getDate().getDayOfMonth()%></span>
                                    <span><%=h.getDate().getMonth().getDisplayName(TextStyle.SHORT, Locale.ENGLISH).toUpperCase()%></span>
                                </div>
                                <div class="min-w-0">
                            ...
                            <% } if (holidayUpcoming.isEmpty()) { %>
                                <p class="text-[11px] font-bold text-slate-300 italic text-center py-10">No upcoming holidays.</p>
                            <% } %>
                        </div>
                    </div>
                </div>
